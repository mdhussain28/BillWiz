...
namespace BillWiz.UI
{
    public partial class S2B : Form
    {
        ...
        private void loadings()
        {
            ...
            Retail_Select.DataSource = eps.GetDataTable("select Shop_name from Retailers_Database");
            Retail_Select.DisplayMember = "Shop_name";
            Retail_Select.ValueMember = "Shop_name";
            Retail_Select.SelectedIndex = -1;
        ...
        private void get_sum()
        {
            ...
            total_amt = Math.Round(total_amt, 4);
        ...
        {
            cid = Convert.ToDouble(cidlbl.Text);
            CustomerType.Enabled = false;
            if(CustomerType.SelectedIndex == 0 )
            {
                MessageBox.Show("Selling at the regular rates");
            }
            else
            {
                MessageBox.Show("Selling at the rates of wholesale");
            }
            if (Retail_Select.SelectedIndex != -1)
            {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (Retail_Select.SelectedIndex != -1)
            {
                ...
                {
                    Sale_Overview expn = new Sale_Overview(Retail_Select.Text, lbl_gross_amt.Text, gst_lbl.Text, lbl_discount.Text, lbl_total_amt.Text, 1, "NA", 0);
                    ...
                    if (res == DialogResult.OK)
                    {
                        ...
                        for (int rowIndex = 0; rowIndex < Items_Grid.Rows.Count; rowIndex++)
                        {
                            ...
                            eps.SetDataTable($"INSERT INTO Retailers_Invoice_items_DB VALUES({ inv_num},'{Retail_Select.Text}','{Convert.ToString(Items_Grid.Rows[rowIndex].Cells[2].Value)}',{ Convert.ToDouble(Items_Grid.Rows[rowIndex].Cells[3].Value)},{ Convert.ToDouble(Items_Grid.Rows[rowIndex].Cells[7].Value)},{ Convert.ToDouble(Items_Grid.Rows[rowIndex].Cells[9].Value) + Convert.ToDouble(Items_Grid.Rows[rowIndex].Cells[10].Value) + Convert.ToDouble(Items_Grid.Rows[rowIndex].Cells[11].Value)},'{date_today}',{ total_amt},{ amtgiven},'{Items_Grid.Rows[rowIndex].Cells[1].Value}',{ Convert.ToDouble(Items_Grid.Rows[rowIndex].Cells[8].Value)},'{Items_Grid.Rows[rowIndex].Cells[4].Value}',{ Convert.ToDouble(Items_Grid.Rows[rowIndex].Cells[5].Value)},{ Convert.ToDouble(Items_Grid.Rows[rowIndex].Cells[13].Value)},{ Convert.ToDouble(Items_Grid.Rows[rowIndex].Cells[6].Value)},{cid})");

                        }

                        eps.SetDataTable($"INSERT INTO All_Sale_Invoices values({inv_num},'{Retail_Select.Text}',{total_amt},{gst},{amtgiven},'{date_today}','Registered')");
                        this.Controls.Clear();
                        this.InitializeComponent();
                        this.loadings();
                        this.Hide();
        ...
        private void Retail_Select_SelectedIndexChanged(object sender, EventArgs e)
        {
            DataTable dt = eps.GetDataTable("select * from Retailers_Database where Shop_name=@name", "@name", Retail_Select.Text);
            foreach (DataRow DR in dt.Rows)
            {
                cidlbl.Text = DR[0].ToString();
                Namelbl.Text = DR[2].ToString();
                phnolbl.Text = DR[3].ToString();
                mailidlbl.Text = DR[4].ToString();
                citylbl.Text = DR[5].ToString();
                cust_gstn = DR[11].ToString();
            }
        }
                    ...
                    if (double.TryParse(row.Cells[7].Value.ToString(), out double amtValue))
                    {
                        if (applyDiscount)
                        {
                            if (Convert.ToDouble(discount_txt.Text) >= 0 && Convert.ToDouble(discount_txt.Text) <= 100)
                            {
                                ...
                                row.Cells[12].Value =Math.Round(discountedAmount + sgst + cgst + igst, 3);
        ...
        private void DrawContent(Graphics graphics, ref RectangleF printArea, PrintPageEventArgs e)
        {
            if (headerprinted == 0)
            {
                ...
                using (Font contentFont = new Font("Verdana", 9))
                {
                    ...
                    string billToInfo = $"Bill To:\nCustomer Name: {Retail_Select.Text}\nCity: {citylbl.Text}\nPhone No. : {phnolbl.Text}\nEmail I'd: {mailidlbl.Text}\nGSTN No.:{cust_gstn}";
...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
        ...
        private void Items_Grid_KeyPress(object sender, KeyPressEventArgs e)
        {
            // Get the currently edited control
            if (Items_Grid.EditingControl is TextBox textBox)
            {
        ...
        private void Items_Grid_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            if (e.Control is TextBox)
            {
               ...
               // textBox.KeyPress -= dataGridViewTextBox_KeyPress;
                ...
                textBox.KeyPress += Items_Grid_KeyPress;
            }
        }
    }
}